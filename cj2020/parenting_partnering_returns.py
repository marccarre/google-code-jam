...
def main() -> None:
    ...
    for i in range(1, testcases + 1):
        ...
        for _ in range(num_activities):
            start, end = [int(t) for t in input().split()]
            activities.append((start, end))
        s = schedule(activities)
        print('Case #%d: %s' % (i, s))


def schedule(activities: List[Tuple[int, int]]) -> str:
    activities = [(a, i) for i, a in enumerate(activities)]
    activities.sort()
    schedule = [''] * len(activities)
    end_cameron, end_jamie = 0, 0  # End-time of the currently assigned activity.
    for (start, end), index in activities:
        if start >= end_cameron:
            end_cameron = end
            schedule[index] = 'C'
        elif start >= end_jamie:
            end_jamie = end
            schedule[index] = 'J'
        else:
            return 'IMPOSSIBLE'
