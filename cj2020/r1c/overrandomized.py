...
def crack(u: int, samples: List[Tuple[int, str]]) -> str:
    D = [None] * 10
    samples.sort()
    for query, response in samples:
        if is_power_of_10(query) and len(str(query)) == len(response):
            D[1] = response[0]
            if query > 1:
                D[0] = response[1]
        elif query < 10:
            if any(response == D[i] for i in range(query)):
                continue
            D[query] = response
        else:
            for c in response:
                ...
                for i, d in enumerate(D):
                    ...
                    if d is None:
                        D[i] = c
        #     lo = 10 ** (len(response) - 1)
        #     hi = min(query, 10**len(response) - 1)
        #     # for i in range(lo, hi + 1):
        #     #     pass
        if all(d is not None for d in D):
            break
    return ''.join(D)
