...
def main() -> None:
    ...
    for i in range(1, testcases + 1):
        east, north, planned_path = input().split()
        out = meet_peppurr((int(east), int(north)), planned_path)
        print('Case #%d: %s' % (i, out))


def meet_peppurr(starting_point: Tuple[int, int], planned_path: str) -> int:
    peppurr = [starting_point]
    for step in planned_path:
        x, y = peppurr[-1]
        if step == 'N':
            peppurr.append((x, y + 1))
        elif step == 'S':
            peppurr.append((x, y - 1))
        elif step == 'E':
