...
def main() -> None:
    ...
    for i in range(1, testcases + 1):
        ...
        for _ in range(num_rows):
            ...
            grid.append(row)
        score = interest_level(grid)
        print('Case #%d: %s' % (i, score))


def interest_level(skills: List[List[int]]) -> int:
    n, m = len(skills), len(skills[0])
    score = 0
    next_skills = [[0] * m for _ in range(n)]
    while True:
        score += sum(cell for row in skills for cell in row)
        for i in range(n):
            for j in range(m):
                neighbours_skills = compassing_neighbours_skills(skills, i, j)
                if neighbours_skills and skills[i][j] < avg(neighbours_skills):
                    next_skills[i][j] = 0
                else:
                    next_skills[i][j] = skills[i][j]
        if skills == next_skills:
            break
        skills = [row[:] for row in next_skills]
    return score
...
def compassing_neighbours_skills(grid: List[List[int]], r: int, c: int) -> List[int]:
    n, m = len(grid), len(grid[0])
    neighbours_skills = []
    # Left:
    for j in range(c - 1, -1, -1):
        if grid[r][j] != 0:
            neighbours_skills.append(grid[r][j])
            break
    # Right:
    ...
    for i in range(r - 1, -1, -1):
        if grid[i][c] != 0:
            neighbours_skills.append(grid[i][c])
            break
    # Bottom:
    for i in range(r + 1, n):
        if grid[i][c] != 0:
            neighbours_skills.append(grid[i][c])
            break
    assert 0 <= len(neighbours_skills) <= 4
    return neighbours_skills
